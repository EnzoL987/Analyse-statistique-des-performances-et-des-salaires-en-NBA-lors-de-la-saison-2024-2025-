...
```{r}
pacman::p_load(tidyverse, knitr, vtable)
```
...
```{r}
nba1 <- nba %>%
  distinct() %>% # Suppressions des doublons parfaits.
  distinct(Player, .keep_all = TRUE) %>% # On enlève les joueurs en
  # double en gardant la première occurence.
  
  # Pour la colonne Salary il faut enlever les signes $ et les virgules, 
  # on va donc utiliser gsub("[\\$,]", "", Salary) qui va remplacer les $
  # et les virgules par du vide. De plus on va aussi utilise
  # as.numeric() afin de convertir le texte en nombre.
  
  mutate(Salary = as.numeric(gsub("[\\$,]", "", Salary))) %>%
  
  # On nettoye les colonnes d'équipes en double en supprimant la colonne
  # Team.y.
  select(-Team.y) %>%
  
  rename(Team = Team.x) # Et on renomme Team.x en simplement Team.

head(nba1)

# On revérifie pour les valeurs manquantes au cas ou.
...
cat("Le salaire moyen dans la ligue va être :", Salaire_moyen, "millions de dollars. \n")
cat("Le salaire médian dans la ligue va être :", Salaire_median,"millions de dollars. \n")
cat("L'écart-type entre les salaires dans la ligue va être de :", Salaire_ecarttype, "millions de dollars.\n")
```
...
tableau_freq <- data.frame(nba_f) %>% 
  ...
  mutate( 
    ...
    Frequence_Pourcentage = Frequence * 100, '%', # On convertit en %.
  )

cat("Tableau des effectifs et fréquences :\n") # Pour introduire.
print(tableau_freq) # Affiche le tableau calculé.
```
...
# Affichage de ces nouvelles valeurs :
head(nba_f %>% select(Salary, FP, salary_hat, residus))
```
...
# Affichage des résultats détaillés du modèle multiple
...
```{r}
pacman::p_load(broom, ggfortify, corrr)
```
