...
Pour répondre à cette problématique, notre étude s'articulera autour de trois grands axes. Dans un premier temps, nous explorerons individuellement les variables clés de notre base (salaires et nationalités) pour en comprendre la distribution. Ensuite, nous déploierons des analyses bivariées et des modèles de régression pour quantifier mathématiquement les liens entre la performance, l'âge et la rémunération. Enfin, nous utiliserons des méthodes statistiques avancées (Analyse en Composantes Principales et Clustering K-means) pour dresser une typologie claire des différents profils de joueurs évoluant dans la ligue.

```{r setup, include=FALSE}
# Configuration globale de knitr pour décider si l'on veut voir le code :
# "echo" affiche ou masque le code en fonction du paramètre YAML
# "warning" et "message" masquent les alertes de chargement des packages
knitr::opts_chunk$set(
  echo = params$afficher_code, 
  warning = FALSE, 
  message = FALSE
)
```
...
cat("Nombre de valeurs manquantes dans la base d'origine :", nombre_na, "\n")
```
...
cat("Nombre de valeurs manquantes dans la base après les modifications :", nombre_na, "\n")
```

```{r}
# On fait maintenant notre grande sélection des données qu'on vous présente dans 
# la partie "Descriprion des données" qui vient juste après.
...
st(
  ...
  title = "Résumé statistique des variables quantitatives que l'on va utiliser",
...
Nous avons fait le choix d'écarter les statistiques trop spécifiques (comme le détail des tirs à 3 points ou les pourcentages aux lancers francs) pour éviter de "noyer" nos algorithmes dans des variables redondantes. En nous concentrant sur les métriques globales reconnues par les analystes sportifs, nous garantissons des modèles statistiques plus robustes et plus lisibles.
...
# Analyse univarié de la variable quantitative Salary 
\hrule 
\vspace{0.4cm}
...
\newpage
# Analyse univarié de la variable qualitative Nationality
\hrule 
\vspace{0.4cm}

## Représentation visuelle et tableau des fréquencese des Top 10 des nationalités en NBA

```{r}
nba_f %>%
  count(Nationality, name = "n") %>%      # Compte le nombre de joueurs par pays
  arrange(desc(n)) %>%                    # Trie du plus grand au plus petit
  slice_head(n = 10) %>%                  # Ne conserve strictement que les 10 premières lignes même si il y a égalité
  
  ggplot(aes(x = reorder(Nationality, n), y = n)) +
  geom_col(fill = "steelblue") +          # geom_col() remplace avantageusement geom_bar(stat = "identity")
  coord_flip() +
  labs(
...
\newpage
# Analyse d’un lien entre variables quantitatives
\hrule 
\vspace{0.4cm}

L'exploration individuelle de nos données a mis en évidence de fortes disparités, notamment une distribution salariale très asymétrique où une élite capte la majorité des revenus. L'objectif de cette nouvelle section est de comprendre ce qui justifie de telles différences. Nous allons donc croiser nos variables pour vérifier si l'impact sur le terrain explique rationnellement l'explosion des contrats.
...
## Contexte : Analyse univariée du Score Fantasy (FP)
...
## Régression linéaire simple : Salaire et Performance
...
## Évaluation du modèle simple et analyse des résidus
...
## Approfondissement : Régression linéaire multiple
...
\newpage
# Analyse multivarié entre deux variables qualitative
\hrule 
\vspace{0.4cm}

## Tableau de contingence et observation graphique

On a choisi ici d'analyser la variable de tranche d'âge avec celle de l'impact sur le terrain lors d'un match tout en exigeant une certaine exigence de niveau pour les joueurs en prenant que ceux avec un Score Fantasy supérieur à la moyenne de la ligue.
...
## Test d'indépendance du Chi-2 et conclusion
...
\newpage
# Analyse multivarié entre variable qualitatif et quantitatif
\hrule 
\vspace{0.4cm}

## Évolution globale du salaire selon l'âge
...
## Dynamique croisée : Performance, Salaire et Tranche d'âge
...
## Visualisation préalable des corrélations
...
## Détermination du nombre d'axes (Screeplot)
...
## Interprétation des composantes et cercles des corrélations
...
## Projection spatiale des joueurs
...
kable(
  ...
  digits = 2, 
  row.names = FALSE, 
  caption = "Coordonnées des 6 premiers joueurs sur les 3 composantes"
)
```
...
\newpage
# Analyse de classification non supervisée
\hrule 
\vspace{0.4cm}

La précédente Analyse en Composantes Principales nous a permis de comprendre les grands "axes" qui structurent la performance en NBA (le volume de jeu d'un côté, le profil défensif/vétéran de l'autre). L'objectif de cette dernière étape est de s'appuyer sur ces similarités statistiques pour regrouper physiquement les joueurs. Grâce à une classification non supervisée, nous allons chercher à diviser la ligue en plusieurs "classes" homogènes, réunissant les athlètes partageant le même profil global de rentabilité et d'impact.
...
## Validation via la Classification Ascendante Hiérarchique (CAH)
...
## Algorithme des K-means et Profilage des groupes
...
## Projection spatiale des classes
